...
One of my favorite ways to study machine learning is through the lens of **probability theory** - the subset of math concerned with modeling randomness and chance. The rules of probability end up providing (what I find are) elegant descriptions of the complex behavior underlying ML.

In this spirit, this post will describe how [logistic regression](https://en.wikipedia.org/wiki/Logistic_regression), a basic binary classification algorithm, can be modeled using concepts from elementary probability theory.

We will start with a basic description of probability-theoretic supervised ML, describe logistic regression within this framework, and show how the commonly used loss function used to train logistic regression models derives from statistical methods such as [maximum likelihood estimation](https://en.wikipedia.org/wiki/Maximum_likelihood_estimation).
...
- $$Y$$ be a random vector specifying the label sampled from the space of all labels. $$Y$$ is a discrete random variable for classification problems and a continuous random variable for regression problems.

Under this framework, **inference** involves predicting the _distribution_ of labels given that we observed a specific feature vector. Mathematically, this means that given that we observe $$\vec{X} = \vec{x}$$, predict the marginal label distribution:

<BlockEquation latex="\mathbb{P}(Y|X=\vec{x})" />

If we want to predict only a _single_ label, we then need sample the label that is most likely to occur in the above distribution.

The marginal label distribution $$\mathbb{P}(Y|X=\vec{x})$$ can be modeled by some arbitrary distribution $$M$$, where:

- $$M$$ is dependent on observed feature vector $$\vec{x}$$
- $$M$$ is parameterized by parameter set $$\theta$$

We call $$M$$ our model. To **train** the model, we must find the values or $$\theta$$ that make $$M$$ optimal for predicting reasonable label distributions.

<BlockEquation latex="\mathbb{P}(Y|X=\vec{x}) = M(\vec{x}; \theta)" />
...
With that out of the way, the logistic regression defined the marginal label distribution as follows:
...
An alternate way of interpreting the logistic regression: it is a model that predicts the probability a given feature vector has a positive label. It does so in two steps:

1. computing the **logit** for the feature vector $$\vec{x}$$ using a linear combination + an intercept: $$\vec{\theta} \cdot \vec{x} + b$$
2. thresholding this logit into the [0, 1] range using the sigmoid (or logistic) function
...
- the **sign of the logit** corresponds to the sign of the predicted label
  - $$\vec{\theta} \cdot \vec{x} + b > 0 \implies \sigma(\vec{\theta} \cdot \vec{x} + b) > 0.5 \implies $$ when logit is positive, model predicts positive label.
  - $$\vec{\theta} \cdot \vec{x} + b < 0 \implies \sigma(\vec{\theta} \cdot \vec{x} + b) < 0.5 \implies $$ when logit is negative, model predicts negative label.
- the **magnitude of the logit** describes the model's confidence in the prediction:
  - if $$|\vec{\theta} \cdot \vec{x} + b| \gg 0 \implies \sigma(\vec{\theta} \cdot \vec{x}+ b)$$ is close to 0 or 1 $$\implies$$ the model predicts a very high or very low probability of a positive label (far from random, high confidence).
  - if $$|\vec{\theta} \cdot \vec{x} + b| \approx 0 \implies \sigma(\vec{\theta} \cdot \vec{x}+ b)$$ is close to 0.5 $$\implies$$ the model predicts close a 50% probability of a positive label (close to random, low confidence).

# Training Logistic Regression Models

To train a logistic regression model, we have to learn the values of $$\vec{\theta}$$ and $$b$$ that allow us to predict the most accurate marginal label distributions. As usual with supervised machine learning, we collect a **training dataset** to determine how close our model is to predicting correct labels.
...
To train a model, we need to be able to determine how "good" certain parameter choices are. One way to do so is by calculating how likely it is that the dataset $$S_n$$ was sampled from the marginal label distribution given by $$\vec{\theta}, b$$.

- if our choice of $$\vec{\theta}, b$$ is good, then we should see that $$\mathbb{P}(Y = y^{(i)} | X = \vec{x}^{(i)}; \vec{\theta}, b)$$ is large, indicating that the labels in $$S_n$$ are likely to be sampled from the marginal label distribution.
- if our choice of $$\vec{\theta}, b$$ is poor, then we may see that $$\mathbb{P}(Y = y^{(i)} | X = \vec{x}^{(i)}; \vec{\theta}, b)$$ is small, indicating that the labels in $$S_n$$ are unlikely to be sampled from the marginal label distribution.

The key in this logic is that $$S_n$$ is our source of truth, and training involves finding parameters that make it as likely as possible to observe that ground truth.

This intuition is formalized as the **likelihood** of model parameters $$\vec{\theta}, b$$, defined as the function $$\mathcal{L}$$ where

<BlockEquation latex="\mathcal{L}(\vec{\theta}, b; S_n) = \mathbb{P}(S_n; \vec{\theta}, b)" />

In other words, $$\mathcal{L}$$ describes how likely it was that the labels in $$S_n$$ were sampled using the parameters $$\vec{\theta}, b$$. As a higher likelihood corresponds to better parameters, we can choose the optimal values for $$\vec{\theta}, b$$ by maximizing $$\mathcal{L}(\vec{\theta}, b; S_n)$$ - a process called **maximum likelihood estimation**.
...
<BlockEquation latex="\argmax_{\vec{\theta}, b} \mathcal{L}(\vec{\theta}, b; S_n) = \argmax_{\vec{\theta}, b} \ell(\vec{\theta}, b; S_n)" />
...
To recap, a logistic regression model is a binary classification model whose marginal distribution of labels is a Bernoulli distribution parameterized by a linear transformation of the feature vector with the parameters $$\vec{\theta}, b$$.
...
While not always the most practical, studying these probability theoretic approaches are a great way to gain an appreciation for the complexity of machine learning strategies, and also understand how our modern neat abstractions of ML algorithms pack away tons of complexity.
